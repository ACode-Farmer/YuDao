...
- (void)setupViews
{
    ...
    _likeLabel.linkTextAttributes = @{NSForegroundColorAttributeName : [UIColor blackColor]};
...
- (void)setLikeItemsArray:(NSArray *)likeItemsArray
{
    ...
    for (int i = 0; i < likeItemsArray.count; i++) {
        LikeItemModel *model = likeItemsArray[i];
        if (i > 0) {
...
- (NSMutableAttributedString *)generateAttributedStringWithLikeItemModel:(LikeItemModel *)model
{
