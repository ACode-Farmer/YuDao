...
#import "YDDBMessageStoreSQL.h"
#import "NSDate+Utilities.h"

@implementation YDDBMessageStore
- (id)init
{
    if (self = [super init]) {
...
- (BOOL)addMessage:(YDMessage *)message
{
    ...
    NSString *fid = @"";
    NSString *subfid;
    if (message.partnerType == YDPartnerTypeUser) {
...
- (void)messagesByUserID:(NSString *)userID partnerID:(NSString *)partnerID fromDate:(NSDate *)date count:(NSUInteger)count complete:(void (^)(NSArray *, BOOL))complete
{
    ...
    [self excuteQuerySQL:sqlString resultBlock:^(FMResultSet *retSet) {
        while ([retSet next]) {
            YDMessage *message = [self y_createDBMessageByFMResultSet:retSet];
...
- (YDMessage *)lastMessageByUserID:(NSString *)userID partnerID:(NSString *)partnerID
{
    ...
    __block YDMessage * message;
    [self excuteQuerySQL:sqlString resultBlock:^(FMResultSet *retSet) {
        while ([retSet next]) {
            message = [self y_createDBMessageByFMResultSet:retSet];
        }
        [retSet close];
    }];
    return message;
...
- (YDMessage *)y_createDBMessageByFMResultSet:(FMResultSet *)retSet
{
    YDMessageType type = [retSet intForColumn:@"msg_type"];
    YDMessage * message = [YDMessage createMessageByType:type];
    ...
    if (message.partnerType == YDPartnerTypeGroup) {
