...
@interface YDMBPTool()<UIGestureRecognizerDelegate>
{
...
#pragma mark - 初始化深色背景
-(void)initBackView{
    bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, screen_width, screen_height)];
...
#pragma mark - 添加时间限制
+ (void) showBriefMessage:(NSString *) message InView:(UIView *) view time:(CGFloat )time{
    hudAddedView = view;
    [self shareManager];
    if (view == nil) {
        view = [[UIApplication sharedApplication] windows].lastObject;
...
+ (void) showBriefAlert:(NSString *) alert time:(CGFloat )time{
    [self showBriefMessage:alert InView:nil time:time];
}
...
-(void)hideBackView{
    ...
    bottomView.frame = CGRectMake(0, 0, screen_width, screen_height);
}
